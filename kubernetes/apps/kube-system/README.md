...
| `namespace` | `kube-system` | Separates this component into the correct tenancy boundary and avoids cross-namespace sprawl. |
| `components` | `['../../components/sops']` | Applies shared Kustomize components used for cross-cutting concerns in this repo. |
| `resources` | `['./namespace.yaml', './cilium/ks.yaml', './coredns/ks.yaml', './metrics-server/ks.yaml...` | Defines which child manifests are composed together as this deployment unit. |


## `namespace.yaml` values

### Document 1: `Namespace` `kube-system`
...
| `metadata.name` | `kube-system` | Keeps object identity stable so upgrades and references continue to work predictably. |
