...
| `namespace` | `devops` | Separates this component into the correct tenancy boundary and avoids cross-namespace sprawl. |
| `components` | `['../../components/sops', '../../components/repos']` | Applies shared Kustomize components used for cross-cutting concerns in this repo. |
| `resources` | `['./gha-runner-scale-set-controller/ks.yaml', './gha-runner-scale-set/ks.yaml', './olla...` | Defines which child manifests are composed together as this deployment unit. |


## `namespace.yaml` values

### Document 1: `Namespace` `devops`
...
| `metadata.name` | `devops` | Keeps object identity stable so upgrades and references continue to work predictably. |
