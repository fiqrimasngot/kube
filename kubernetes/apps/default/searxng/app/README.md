...
### Document 1: `HelmRelease` `searxng`
...
| `metadata.name` | `searxng` | Keeps object identity stable so upgrades and references continue to work predictably. |
...
| `configMapGenerator.[0].name` | `searxng-configmap` | Set intentionally to match this repository's desired-state for this component. |
...
| `metadata.name` | `searxng` | Keeps object identity stable so upgrades and references continue to work predictably. |
| `stringData.SEARXNG_SECRET` | `ENC[AES256_GCM,data:Js5LePcN1sdQSWuth3eQT3ae0xSYQcbqrWOLYvcfJZk=,iv:qFCBEZQ+bpI3SGoO58T...` | Set intentionally to match this repository's desired-state for this component. |
...
| `sops.age.[0].enc` | `-----BEGIN AGE ENCRYPTED FILE----- YWdlLWVuY3J5cHRpb24ub3JnL3YxCi0+IFgyNTUxOSAxOHhRZDEx...` | Set intentionally to match this repository's desired-state for this component. |
| `sops.lastmodified` | `2026-02-17T11:15:05Z` | Numeric value tuned for current workload size, reliability, and cluster capacity. |
| `sops.mac` | `ENC[AES256_GCM,data:+kx9yJB1c1B5VAGNZJYEmE0O4cSFzm4aGdufv2QvVZk6AcJ1D6y+bS9wHmERPhAZ3ki...` | Set intentionally to match this repository's desired-state for this component. |
