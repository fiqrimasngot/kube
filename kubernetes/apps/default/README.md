...
| `namespace` | `default` | Separates this component into the correct tenancy boundary and avoids cross-namespace sprawl. |
...
| `resources` | `['./namespace.yaml', './actual/ks.yaml', './mealie/ks.yaml', './shlink/ks.yaml', './hom...` | Defines which child manifests are composed together as this deployment unit. |


## `namespace.yaml` values

### Document 1: `Namespace` `default`
...
| `metadata.name` | `default` | Keeps object identity stable so upgrades and references continue to work predictably. |
