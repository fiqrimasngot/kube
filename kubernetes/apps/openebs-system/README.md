...
| `namespace` | `openebs-system` | Separates this component into the correct tenancy boundary and avoids cross-namespace sprawl. |
| `components` | `['../../components/sops']` | Applies shared Kustomize components used for cross-cutting concerns in this repo. |
| `resources` | `['./namespace.yaml', './openebs/ks.yaml']` | Defines which child manifests are composed together as this deployment unit. |


## `namespace.yaml` values

### Document 1: `Namespace` `openebs-system`
...
| `metadata.name` | `openebs-system` | Keeps object identity stable so upgrades and references continue to work predictably. |
