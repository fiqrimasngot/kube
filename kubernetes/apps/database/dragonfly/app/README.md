...
### Document 1: `HelmRelease` `dragonfly-operator`
...
| `metadata.name` | `dragonfly-operator` | Keeps object identity stable so upgrades and references continue to work predictably. |
...
| `resources` | `['https://raw.githubusercontent.com/dragonflydb/dragonfly-operator/v1.1.6/manifests/crd...` | Defines which child manifests are composed together as this deployment unit. |


## `rbac.yaml` values

### Document 1: `ClusterRole` `dragonfly-operator`
...
| `metadata.name` | `dragonfly-operator` | Keeps object identity stable so upgrades and references continue to work predictably. |
| `rules` | `list[7]` | Set intentionally to match this repository's desired-state for this component. |

### Document 2: `ClusterRoleBinding` `dragonfly-operator`
...
| `roleRef.name` | `dragonfly-operator` | Set intentionally to match this repository's desired-state for this component. |
...
| `subjects.[0].name` | `dragonfly-operator` | Set intentionally to match this repository's desired-state for this component. |
| `subjects.[0].namespace` | `database` | Separates this component into the correct tenancy boundary and avoids cross-namespace sprawl. |
