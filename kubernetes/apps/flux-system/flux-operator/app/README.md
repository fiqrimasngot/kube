...
### Document 1: `HelmRelease` `flux-operator`
...
| `metadata.name` | `flux-operator` | Keeps object identity stable so upgrades and references continue to work predictably. |
| `spec.chartRef.kind` | `OCIRepository` | Binds this release to a known source object so artifact resolution is deterministic. |
| `spec.chartRef.name` | `flux-operator` | Binds this release to a known source object so artifact resolution is deterministic. |
| `spec.interval` | `1h` | Uses a reconcile interval that balances configuration freshness with API/controller load. |
| `spec.values.serviceMonitor.create` | `True` | Enabled because this behavior is required for reliable operation in this environment. |
...
### Document 1: `OCIRepository` `flux-operator`
...
| `spec.url` | `oci://ghcr.io/controlplaneio-fluxcd/charts/flux-operator` | Set intentionally to match this repository's desired-state for this component. |
