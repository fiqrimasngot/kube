...
| `namespace` | `security` | Separates this component into the correct tenancy boundary and avoids cross-namespace sprawl. |
| `components` | `['../../components/sops']` | Applies shared Kustomize components used for cross-cutting concerns in this repo. |
| `resources` | `['./namespace.yaml', './authentik/ks.yaml']` | Defines which child manifests are composed together as this deployment unit. |


## `namespace.yaml` values

### Document 1: `Namespace` `security`
...
| `metadata.name` | `security` | Keeps object identity stable so upgrades and references continue to work predictably. |
