...
| `namespace` | `storage` | Separates this component into the correct tenancy boundary and avoids cross-namespace sprawl. |
...
| `resources` | `['./namespace.yaml', './nfs-server/ks.yaml', './minio/ks.yaml', './csi-driver-nfs/ks.ya...` | Defines which child manifests are composed together as this deployment unit. |


## `namespace.yaml` values

### Document 1: `Namespace` `storage`
...
| `metadata.name` | `storage` | Keeps object identity stable so upgrades and references continue to work predictably. |
