...
### Document 1: `HelmRelease` `immich-machine-learning`
...
| `metadata.name` | `immich-machine-learning` | Keeps object identity stable so upgrades and references continue to work predictably. |
...
| `spec.dependsOn.[0].name` | `immich-server` | Set intentionally to match this repository's desired-state for this component. |
| `spec.dependsOn.[0].namespace` | `media` | Separates this component into the correct tenancy boundary and avoids cross-namespace sprawl. |
