...
### Document 1: `HelmRelease` `k8s-gateway`
...
| `metadata.name` | `k8s-gateway` | Keeps object identity stable so upgrades and references continue to work predictably. |
| `spec.chartRef.kind` | `OCIRepository` | Binds this release to a known source object so artifact resolution is deterministic. |
| `spec.chartRef.name` | `k8s-gateway` | Binds this release to a known source object so artifact resolution is deterministic. |
| `spec.interval` | `1h` | Uses a reconcile interval that balances configuration freshness with API/controller load. |
| `spec.values.fullnameOverride` | `k8s-gateway` | Set intentionally to match this repository's desired-state for this component. |
| `spec.values.domain` | `${SECRET_DOMAIN}` | Set intentionally to match this repository's desired-state for this component. |
| `spec.values.ttl` | `1` | Numeric value tuned for current workload size, reliability, and cluster capacity. |
| `spec.values.service.type` | `LoadBalancer` | Set intentionally to match this repository's desired-state for this component. |
| `spec.values.service.port` | `53` | Uses the network port expected by clients, probes, or upstream services. |
| `spec.values.service.externalTrafficPolicy` | `Cluster` | Set intentionally to match this repository's desired-state for this component. |
| `spec.values.watchedResources` | `['HTTPRoute', 'Service']` | Set intentionally to match this repository's desired-state for this component. |
...
### Document 1: `OCIRepository` `k8s-gateway`
...
| `spec.ref.tag` | `3.2.8` | Locks the application version to avoid unexpected upstream changes. |
| `spec.url` | `oci://ghcr.io/k8s-gateway/charts/k8s-gateway` | Set intentionally to match this repository's desired-state for this component. |
