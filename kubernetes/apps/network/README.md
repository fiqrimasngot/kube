...
| `namespace` | `network` | Separates this component into the correct tenancy boundary and avoids cross-namespace sprawl. |
...
| `resources` | `['./namespace.yaml', './cloudflare-dns/ks.yaml', './cloudflare-tunnel/ks.yaml', './envo...` | Defines which child manifests are composed together as this deployment unit. |


## `namespace.yaml` values

### Document 1: `Namespace` `network`
...
| `metadata.name` | `network` | Keeps object identity stable so upgrades and references continue to work predictably. |
